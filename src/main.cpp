...
static void scenario_4() {
    std::vector<SimClient> clients = {
        {1, "A", 2},   // starts at 2, will change
        {2, "B", 3}
    };

    // Build MODIFY events: client A's ticket count drawn uniformly from [2,12]
    // and updated every 2 quanta.
    std::mt19937 rng(99);
...
static void scenario_6() {
    // Start with clients A and B.
    std::vector<SimClient> clients = {
        {1, "A", 6},
        {2, "B", 4}
    };

    // Client C joins at quantum 200, leaves at quantum 400.
    // Client D joins at quantum 300.
    std::vector<DynamicEvent> events = {
...
static void scenario_7() {
    ...
    // Statistical equivalence check: wins within 10% of each other.
    bool all_close = true;
...
// Two stable clients A(4) and B(4) compete throughout; three churning
// clients C1/C2/C3(2 each) cycle in and out every 20 quanta.
// -----------------------------------------------------------------------
static void scenario_8() {
    ...
    // Churning clients (ids 10,11,12): each active for 10q then idle for 10q.
    // Staggered phase offsets so events don't all fire simultaneously.
    std::vector<DynamicEvent> events;
...
// P1 = 3 threads T1a/T1b/T1c(2 each, total 6) vs P2 = T2(6).
// Equal totals => ~50/50. At q501 P1 spawns T1d(2) => inflation to 8:6.
// -----------------------------------------------------------------------
static void scenario_9() {
    ...
    // Aggregate per-process wins split at the inflation point.
...
// 200 independent trials at each of 5 log-spaced n values for the
// 7:3 ratio. Shows E[AbsErr] ∝ √n and E[TpErr] ∝ 1/√n.
// -----------------------------------------------------------------------
static void scenario_10() {
    ...
    // E[|X-μ|] = σ·√(2/π) for a normal approximation (half-normal mean).
    // For a binomial at moderate n this is tight, so:
    // E[AbsErr]/√n ≈ √(p·(1-p)) · √(2/π) ≈ 0.458 · 0.798 ≈ 0.366
